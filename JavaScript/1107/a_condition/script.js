...
//! 2. if else문
// else문은 if문과 함께 사용, if 조건이 거짓(false)일 경우 else 뒤의 코드 블록이 실행
...
// else if문은 if문의 조건이 거짓이고, else if문의 조건이 참일 때 실행
...
// swich문은 여러가지 가능한 케이스 중 하나를 선택하여
...
// break문(필수X)
// : 이전의 선택이 표현식이나 값과 일치한다면, 브라우저는 해당 코드 블록에서 실행을 멈추고, switch문 아래에 있는 코드로 이동
...
// and 연산자는 조건식이 둘 이상일 경우, 하나만 false여도 최종 결괏값이 false

// >> 첫 번째 조건식이 false면, 두번째 조건식은 체크하지 않고 바로 false
// and 연산자 사용 시 false가 될 확률이 높은 조건을 첫 번째 조건식으로 사용하는 것을 권장

// or 연산자는 조건식이 둘 이상일 경우, 하나만 true여도 최종 결괏값이 true

// >> true가 될 확률이 높은 조건식을 첫번째 조건식으로 사용하는 것을 권장